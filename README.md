...
유량 상쇄란, 쉽게 말해 유량이 흐르는 반대 방향으로 음의 유량을 흘려 보냄으로써 유량을 상쇄 시키는 것을 의미한다.

위 그래프를 이용해 설명하겠다.

S → a → b → T, S → b → a → T 로 유량을 흘려 보낼때, 겹치는 정점의 노드인 a → b의 유량은 f(a,b)로 나타낼 수 있으며 b → a의 유량은 f(b,a)로 나타낼 수 있다.
...
S A B T 그래프 예를 들면서 ~~ 설명 하고 사진도 하나 넣고 알고리즘 소스 구현
